...
- **Production cutover runbook (not done yet):** Server still runs SQLite. Steps to cut over: (1) provision PG, run `alembic upgrade head`; (2) migrate data: `pgloader sqlite:///data/insider_tracker.db postgresql://...`; (3) set `DATABASE_URL` in server `.env`; (4) `git pull && sudo systemctl restart insider-tracker.service`; (5) smoke test `/healthz`, `/`, `/run-log`; (6) decommission SQLite file. Do NOT deploy branch without `DATABASE_URL` set — first DB touch raises `RuntimeError`.
