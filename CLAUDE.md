...
- **`OnCalendar` named-timezone suffix silently breaks the timer on systemd < 247:** `insider-prices.timer` originally used `OnCalendar=Mon..Fri 21:00 America/New_York`. Named-timezone suffixes in `OnCalendar` were only added in systemd v247 (Ubuntu 20.10+). On older systemd the expression fails to compute a next elapse — `systemctl list-timers` shows `Trigger: n/a`, the job never fires, and the prices heartbeat goes stale with no error in `journalctl` (the timer just sits idle). Every other timer worked because they use bare/`UTC` times. Fix: the timer now uses `OnCalendar=Mon-Fri 01:00 UTC` (matches `insider-backup.timer`'s proven `UTC` form and CLAUDE.md's documented "01:00 UTC"). When adding a timer, never use a named TZ suffix unless the server's systemd is confirmed ≥ 247 — prefer an explicit `UTC` time. Diagnose with `systemd-analyze calendar "<expr>"` (shows "Next elapse" if valid).
- **`deploy.sh` must install ALL units in `schedule/`, or they drift:** The original `deploy.sh --setup` only installed the web + ingest units; `insider-prices`, `insider-backup` (and the nightly ingest units, which aren't in the repo at all) were hand-installed on the server and never re-synced from git. That's how a stale unit file survives a `git pull` + redeploy. `deploy.sh` now `cp`s and `enable --now`s the prices and backup units too. **Recovery for the prices outage (run on the server after `git pull`):** `sudo cp schedule/insider-prices.timer schedule/insider-prices.service /etc/systemd/system/ && sudo systemctl daemon-reload && sudo systemctl enable --now insider-prices.timer && sudo systemctl start insider-prices.service && systemctl list-timers insider-prices.timer` — confirm `NEXT` shows a future timestamp (not `n/a`). The standard redeploy path (`deploy.sh` without `--setup`) only restarts the web service — it does NOT re-copy units, so unit-file changes always need this manual `cp` + `daemon-reload` step on the server.
