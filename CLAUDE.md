...
- **`transaction_date` is a `date` object in PG, was a string in SQLite:** Any code that compares `transaction_date` lexicographically to a string (e.g. `td >= "2025-01-01"`) needs explicit `.isoformat()` first. `_batch_cluster_counts` and `alerts.check_and_send_signals` both have explicit normalization to ISO strings.
- **`health_check.py`, `backtest.py`, `generate_lc_chart.py`, `generate_signals_chart.py` were intentionally NOT ported:** They still use `sqlite3` directly against the SQLite file. Each has a `TODO(pg-cutover)` comment at the top. They must be ported before the SQLite file is decommissioned. `health_check.py` is called from `ingest.py` with a psycopg connection — that call will fail at cutover until ported.
- **No production cutover has happened yet:** The server still runs SQLite. Imports succeed without `DATABASE_URL` (the pool is lazy), but the first request that touches the DB will raise `RuntimeError: DATABASE_URL environment variable not set`. Deploying this branch without first standing up PG and setting `DATABASE_URL` will break every page that queries the DB. Coordinate the cutover: data migration (pgloader/CSV), `alembic upgrade head`, env var rollout, deploy, smoke test.
